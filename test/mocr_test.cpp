...
TEST(destroy_test, valid)
{
    mocr_ctx *ctx = mocr_init(DEFAULT_MODEL, 0);
    ASSERT_NE(ctx, nullptr);
    ASSERT_EQ(mocr_destroy(ctx), 0);
}

TEST(destroy_test, null)
{
    mocr_ctx *ctx = mocr_init(DEFAULT_MODEL, 0);
    ASSERT_NE(ctx, nullptr);
    ASSERT_EQ(mocr_destroy(ctx), 0);
...
protected:
    ...
    void test_file(const char *path, const char *expected_text)
    {
        ...
        ASSERT_STREQ(text, expected_text);
        ASSERT_EQ(mocr_free(text), 0);
    }
...
TEST(free_test, null_ptr)
{
    ASSERT_EQ(mocr_free(nullptr), 0);
}